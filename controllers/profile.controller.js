...
export const createProfile = asynchandler(async (req, res) => {
  ...
  try {
    ...
    const social1 =
      social.length > 0
        ? await cloudinary.uploader.upload(social[0].path)
        : { secure_url: "", public_id: "" };
    const company1 =
      company.length > 0
        ? await cloudinary.uploader.upload(company[0].path)
        : { secure_url: "", public_id: "" };
    const personal1 =
      personal.length > 0
        ? await cloudinary.uploader.upload(personal[0].path)
        : { secure_url: "", public_id: "" };
    ...
    res
      .status(201)
      .json({ message: "Profile created successfully", savedProfile });
  } catch (error) {
...
export const getuserprofile = asynchandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Profile founded", existingProfile });
  } catch (error) {
    console.error("Error fetching profile:", error.message);
    res
      .status(500)
...
export const addapp = asynchandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: "App added successfully", profile: savedProfile });
  } catch (error) {
    // Handle errors
    return res
      .status(500)
...
export const updateapp = asynchandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "App updated successfully", profile: savedProfile });
  } catch (error) {
    // Handle errors
    return res
      .status(500)
...
export const deleteapp = asynchandler(async (req, res) => {
  ...
  try {
    ...
    const appIndex = profile.apps.findIndex(
    ...
    return res
      .status(200)
      .json({ message: "App deleted successfully", profile: savedProfile });
  } catch (error) {
    // Handle errors
    return res
      .status(500)
...
export const sendrequest = asynchandler(async (req, res) => {
  ...
  try {
    ...
    if (!profile) {
      return res
        .status(404)
        .json({ message: "Target profile not found or is private" });
    }
    ...
    const existingConnection = profile.connections.find(
      (connection) =>
        ...
        connection.profileid.toString() === userprofileid
    );
...
export const getpendingrequests = asynchandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Pending requests found", pendingRequests });
  } catch (error) {
    // Handle errors
    return res
      .status(500)
...
export const getacceptedrequests = asynchandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Accepted requests found", acceptedRequests });
  } catch (error) {
    // Handle errors
    return res
      .status(500)
...
export const changepersonalstatus = asynchandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Profile updated successfully", profile: savedProfile });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Something went wrong", error: error.message });
...
export const changeprivatestatus = asynchandler(async (req, res) => {
  const { id, status } = req.body;
  try {
    if (!id) {
      return res.status(400).json({ message: "Missing required fields" });
    ...
    existingProfile.private = Boolean(status);
    const savedProfile = await existingProfile.save();
    return res
      .status(200)
      .json({ message: "Profile updated successfully", profile: savedProfile });
...
export const getprofile = asynchandler(async (req, res) => {
  ...
  try {
    ...
    const existingProfile = await Profile.findById(id).populate(
      "connections.userid"
    );
    if (!existingProfile) {
      return res.status(404).json({ message: "Profile not found" });
    ...
    if (existingProfile.personal) {
      const isConnected = existingProfile.connections.some(
        (connection) =>
          connection.userid &&
          connection.userid._id.toString() === userid &&
          connection.status === "accepted"
      );

      if (!isConnected) {
        const { apps, connections, ...dataWithoutAppsAndConnections } =
